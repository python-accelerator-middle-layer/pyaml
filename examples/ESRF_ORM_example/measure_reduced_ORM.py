...
orm = OrbitResponseMatrix(cfg=ORM_ConfigModel(bpm_array_name='BPM', hcorr_array_name='HCorr',
                                              vcorr_array_name='VCorr', corrector_delta=1e-6),
                          element_holder=element_holder)

hcorr = element_holder.get_magnets('HCorr')
vcorr = element_holder.get_magnets('VCorr')
corrector_names = hcorr['SJ2A*'].names() + hcorr['SF2A*'].names() + hcorr['SI2A*'].names()\
                + vcorr['SJ2A*'].names() + vcorr['SF2A*'].names() + vcorr['SI2A*'].names()
