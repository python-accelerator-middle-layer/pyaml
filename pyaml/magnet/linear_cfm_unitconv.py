...
from ..configuration.csvcurve import CSVCurve
...
class ConfigModel(BaseModel):
    ...
    curves: list[Curve]
    """Calibration curves used for interpolation, 1 curve per function"""
    ...
    """List of power converter devices to apply currrents (can be different from number of function)"""
    matrix: Matrix
    """n x m matrix (n rows for n function , m columns for m currents) to handle cross depecdancies between functions"""
    units: list[str]
...
class LinearCFMagnetUnitConv(UnitConv):
    """
    Class providing a simple linear model for combined function magnets
    """
    def __init__(self, cfg:ConfigModel):
        ...
        if len(self._calibration_factors) != self._nbFunction:
            raise Exception("calibration_factors does not have the expected " \
            "number of items ({self._nbFunction} expected)")
        if len(self._calibration_offsets) != self._nbFunction:
            raise Exception("calibration_offsets does not have the expected " \
            "number of items ({self._nbFunction} expected)")
        if len(cfg.units) != self._nbFunction:
            raise Exception("units does not have the expected " \
            "number of items ({self._nbFunction} expected)")
        if len(cfg.curves) != self._nbFunction:
            raise Exception("curves does not have the expected " \
            "number of items ({self._nbFunction} expected)")
    ...
    def compute_currents(self,strengths:np.array) -> np.array:
        ...
        for idx,c in enumerate(self._cfg.curves):
            _pI[idx] = np.interp(strengths[idx], c[:, 1], c[:, 0]) * self._brho
    ...
    def compute_strengths(self,currents:np.array) -> np.array:
        ...
        for idx,c in enumerate(self._cfg.curves):
            _strength[idx] = np.interp(_pI[idx], c[:, 0], c[:, 1]) / self._brho
        return _strength
