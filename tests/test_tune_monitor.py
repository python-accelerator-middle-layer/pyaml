...
def test_simulator_tune_monitor(accelerator_from_config):
    sr: Accelerator = accelerator_from_config("tests/config/tune_monitor.yaml", ignore_external=True)
...
def test_controlsystem_tune_monitor(install_test_package, accelerator_from_config):
    sr: Accelerator = accelerator_from_config("tests/config/tune_monitor.yaml")
