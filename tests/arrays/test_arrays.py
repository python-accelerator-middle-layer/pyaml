...
def test_arrays(install_test_package):
    ...
    # Using aggregator
    pos = sr.design.diagnostic.bpms.get("BPMS").positions.get()
    ...
    pos_h = sr.design.diagnostic.bpms.get("BPMS").h.get()
    pos_v = sr.design.diagnostic.bpms.get("BPMS").v.get()
    ...
    sr.design.diagnostic.bpm.get("BPM_C04-01").offset.set([0.1, 0.2])
    sr.design.diagnostic.bpm.get("BPM_C04-02").offset.set([0.3, 0.4])
    pos = sr.design.diagnostic.bpms.get("BPMS").positions.get()
    ...
    sr.design.diagnostic.bpm.get("BPM_C04-01").offset.set([0.0, 0.0])
    sr.design.diagnostic.bpm.get("BPM_C04-02").offset.set([0.0, 0.0])

    # No aggregator
    bpms = []
    for b in sr.design.diagnostic.bpms.get("BPMS"):
        bpms.append(b)
    ...
    allBpms = BPMArray("AllBPMs", sr.design.diagnostic.bpm.all())
    ...
    bpmsLive = BPMArray("", sr.live.diagnostic.bpm.all())
    ...
    v = sr.design.diagnostic.bpms.get("emptyBPM").positions.get()  # Ensure good attach
    assert np.shape(v) == (0,)
