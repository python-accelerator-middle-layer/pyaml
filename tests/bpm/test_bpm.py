...
def test_simulator_bpm_tilt():
    ...
    bpm = sr.design.diagnostic.bpm.get("BPM_C01-01")
...
def test_simulator_bpm_position_with_bad_corrector_strength():
    sr: Accelerator = Accelerator.load("tests/config/bpms.yaml", ignore_external=True)
    sr.design.get_lattice().disable_6d()
    bpm1 = sr.design.diagnostic.bpm.get("BPM_C01-01")
    bpm_simple = sr.design.diagnostic.bpm.get("BPM_C01-02")
    bpm3 = sr.design.diagnostic.bpm.get("BPM_C01-03")
